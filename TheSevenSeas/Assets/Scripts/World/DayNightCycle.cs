...
public class DayNightCycle : MonoBehaviour
{
    ...
    [Header("Fog Color")]
    [Tooltip("Enable fog color changes based on time of day")]
    public bool manageFogColor = true;
    
    [Tooltip("Fog color at midnight (fully black)")]
    public Color midnightFogColor = Color.black;
    
    [Tooltip("Fog color at midday (white)")]
    public Color middayFogColor = Color.white;
    
    [Tooltip("Fog color at sunrise")]
    public Color sunriseFogColor = new Color(1f, 0.7f, 0.5f);
    
    [Tooltip("Fog color at sunset")]
    public Color sunsetFogColor = new Color(1f, 0.6f, 0.4f);
    ...
    void Start()
    {
        ...
        if (manageFogDensity || manageFogColor)
        {
    ...
    void UpdateFogColor()
    {
        ...
        // Dawn: 4-7 (transition from black to sunrise colors)
        if (currentTimeOfDay >= 4f && currentTimeOfDay < 7f)
        {
            float t = (currentTimeOfDay - 4f) / 3f;
            targetColor = Color.Lerp(midnightFogColor, sunriseFogColor, t);
        }
        // Morning to Midday: 7-12 (sunrise to white)
        else if (currentTimeOfDay >= 7f && currentTimeOfDay < 12f)
        {
            float t = (currentTimeOfDay - 7f) / 5f;
            targetColor = Color.Lerp(sunriseFogColor, middayFogColor, t);
        }
        // Midday to Afternoon: 12-17 (white stays white)
        else if (currentTimeOfDay >= 12f && currentTimeOfDay < 17f)
        {
            targetColor = middayFogColor;
        }
        // Sunset: 17-19 (white to sunset colors)
        else if (currentTimeOfDay >= 17f && currentTimeOfDay < 19f)
        {
            float t = (currentTimeOfDay - 17f) / 2f;
            targetColor = Color.Lerp(middayFogColor, sunsetFogColor, t);
        }
        // Dusk: 19-22 (sunset colors to black)
        else if (currentTimeOfDay >= 19f && currentTimeOfDay < 22f)
        {
            float t = (currentTimeOfDay - 19f) / 3f;
            targetColor = Color.Lerp(sunsetFogColor, midnightFogColor, t);
        }
        // Night: 22-4 (fully black)
        else
        {
            targetColor = midnightFogColor;
        }
    ...
    void OnGUI()
    {
        if (showDebugInfo)
        {
            ...
            if (manageFogColor)
            {
                GUI.Label(new Rect(10, 60, 300, 30), $"Fog Color: {RenderSettings.fogColor}", style);
            }
            
            if (directionalLight != null)
            {
                GUI.Label(new Rect(10, 85, 300, 30), $"Light Intensity: {directionalLight.intensity:F2}", style);
            }
        }
    }
